...
def _get_output(
    output: (
        type[complex | npc.number] | np.dtype[npc.number] | str | onp.ToComplexND | None
    ),
...
def _get_output(
    output: (
        type[complex | npc.complexfloating]
        | np.dtype[npc.complexfloating]
        | str
        | onp.ToJustComplexND
        | None
    ),
...
#
def _check_axes(
    axes: op.CanIndex | Iterable[op.CanIndex], ndim: int
) -> tuple[int, ...]: ...
