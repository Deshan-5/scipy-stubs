...
_InexactT_co = TypeVar(
    "_InexactT_co", bound=npc.inexact, default=np.float64, covariant=True
)
_EngineT_co = TypeVar(
    "_EngineT_co", bound=QMCEngine[npc.inexact], default=Sobol, covariant=True
)
...
class _QMCDistribution(Generic[_EngineT_co]):
    ...
    def __init__(
        ...
        /,
        *,
        engine: _EngineT_co | None = None,
        rng: onp.random.ToRNG | None = None,
    ) -> None: ...
    def random(
        self: _QMCDistribution[QMCEngine[_InexactT]], /, n: onp.ToJustInt = 1
...
class QMCEngine(abc.ABC, Generic[_InexactT_co]):
    ...
    optimization_method: Callable[
        Concatenate[onp.ArrayND[_InexactT_co], ...], onp.Array2D[_InexactT_co]
    ]
    ...
    @abc.abstractmethod
    def _random(
        self, /, n: onp.ToJustInt = 1, *, workers: onp.ToJustInt = 1
...
def check_random_state(
    seed: int | npc.integer | numbers.Integral | None = None,
) -> np.random.Generator: ...
@overload
...
def update_discrepancy(
    x_new: onp.ToJustFloat1D, sample: onp.ToJustFloat2D, initial_disc: opt.AnyFloat
) -> float: ...
...
def _select_optimizer(
    optimization: _MethodQMC | None, config: Mapping[str, object]
) -> _Optimizer | None: ...
def _random_cd(
    best_sample: _FloatArrayT,
    n_iters: onp.ToInt,
    n_nochange: onp.ToInt,
    rng: onp.random.RNG,
) -> _FloatArrayT: ...
def _l1_norm(sample: onp.ToJustFloat2D) -> float | np.float64: ...
def _lloyd_iteration(
    sample: _FloatArrayT, decay: onp.ToFloat, qhull_options: str | None
) -> _FloatArrayT: ...
...
def _perturb_discrepancy(
    sample: onp.Array2D[npc.integer | np.bool_],
    i1: op.CanIndex,
    i2: op.CanIndex,
    k: op.CanIndex,
    disc: onp.ToFloat,
...
def _van_der_corput_permutation(
    base: op.CanIndex, *, random_state: onp.random.ToRNG | None = None
) -> onp.Array2D[np.int_]: ...
@overload
def _van_der_corput_permutation(
