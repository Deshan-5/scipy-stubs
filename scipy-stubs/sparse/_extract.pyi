...
_ToDense: TypeAlias = (
    onp.CanArrayND[_SCT] | Seq[_SCT] | Seq[Seq[_SCT] | onp.CanArrayND[_SCT]]
)
...
#
def find(
    A: _spbase[_SCT] | _ToDense,
) -> tuple[onp.Array1D[np.int32], onp.Array1D[np.int32], onp.Array1D[_SCT]]: ...

# NOTE: `tril` and `triu` have identical signatures
@overload  # sparray -> coo_array (default)
def tril(
    A: sparray[_SCT], k: int = 0, format: Literal["coo"] | None = None
...
def tril(
    A: sparray[_SCT], k: int = 0, *, format: Literal["bsr"]
) -> bsr_array[_SCT]: ...
...
def tril(
    A: sparray[_SCT], k: int = 0, *, format: Literal["csc"]
) -> csc_array[_SCT]: ...
@overload  # sparray -> csr_array (positional)
def tril(
    A: sparray[_SCT], k: int, format: Literal["csr"]
) -> csr_array[_SCT, tuple[int, int]]: ...
@overload  # sparray -> csr_array (keyword)
def tril(
    A: sparray[_SCT], k: int = 0, *, format: Literal["csr"]
) -> csr_array[_SCT, tuple[int, int]]: ...
...
def tril(
    A: sparray[_SCT], k: int = 0, *, format: Literal["dia"]
) -> dia_array[_SCT]: ...
@overload  # sparray -> dok_array (positional)
def tril(
    A: sparray[_SCT], k: int, format: Literal["dok"]
) -> dok_array[_SCT, tuple[int, int]]: ...
@overload  # sparray -> dok_array (keyword)
def tril(
    A: sparray[_SCT], k: int = 0, *, format: Literal["dok"]
) -> dok_array[_SCT, tuple[int, int]]: ...
...
def tril(
    A: sparray[_SCT], k: int = 0, *, format: Literal["lil"]
) -> lil_array[_SCT]: ...
@overload  # spmatrix | array-like -> coo_matrix (default)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int = 0, format: Literal["coo"] | None = None
) -> coo_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> bsr_matrix (positional)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int, format: Literal["bsr"]
) -> bsr_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> bsr_matrix (keyword)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int = 0, *, format: Literal["bsr"]
) -> bsr_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> csc_matrix (positional)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int, format: Literal["csc"]
) -> csc_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> csc_matrix (keyword)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int = 0, *, format: Literal["csc"]
) -> csc_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> csr_matrix (positional)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int, format: Literal["csr"]
) -> csr_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> csr_matrix (keyword)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int = 0, *, format: Literal["csr"]
) -> csr_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> dia_matrix (positional)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int, format: Literal["dia"]
) -> dia_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> dia_matrix (keyword)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int = 0, *, format: Literal["dia"]
) -> dia_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> dok_matrix (positional)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int, format: Literal["dok"]
) -> dok_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> dok_matrix (keyword)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int = 0, *, format: Literal["dok"]
) -> dok_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> lil_matrix (positional)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int, format: Literal["lil"]
) -> lil_matrix[_SCT]: ...
@overload  # spmatrix | array-like -> lil_matrix (keyword)
def tril(
    A: spmatrix[_SCT] | _ToDense[_SCT], k: int = 0, *, format: Literal["lil"]
) -> lil_matrix[_SCT]: ...
@overload  # catch-all
def tril(
    A: _spbase[_SCT] | onp.ToComplexND, k: int = 0, *, format: _Format | None = None
) -> _spbase[_SCT, tuple[int, int]]: ...
