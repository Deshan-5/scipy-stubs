...
_ToF32Mat: TypeAlias = (
    _Sparse2D[np.float32] | nptc.CanArray[tuple[Any, ...], np.dtype[np.float32]]
)
_ToF64Mat: TypeAlias = (
    _Sparse2D[np.float64 | npc.integer] | onp.ToInt2D | onp.ToJustFloat64_2D
)
_ToC64Mat: TypeAlias = (
    _Sparse2D[np.complex64] | nptc.CanArray[tuple[Any, ...], np.dtype[np.complex64]]
)
_ToC128Mat: TypeAlias = _Sparse2D[np.complex128] | onp.ToJustComplex128_2D

_ToFloatMat: TypeAlias = (
    _Sparse2D[npc.floating | npc.integer | np.bool_] | onp.ToFloat2D
)
_ToFloatMatStrict: TypeAlias = (
    _Sparse2D[npc.floating | npc.integer | np.bool_] | onp.ToFloatStrict2D
)
...
class _SuperLU_solve(Protocol[_NumberT_contra, _InexactT_co]):
    @overload
    def __call__(
        self, rhs: onp.Array1D[_NumberT_contra | np.bool_]
    ) -> onp.Array1D[_InexactT_co]: ...
    @overload
    def __call__(
        self, rhs: onp.Array2D[_NumberT_contra | np.bool_]
    ) -> onp.Array2D[_InexactT_co]: ...
    @overload
    def __call__(
        self, rhs: onp.ArrayND[_NumberT_contra | np.bool_]
    ) -> onp.ArrayND[_InexactT_co]: ...
...
def use_solver(
    *, useUmfpack: bool = True, assumeSortedIndices: bool = False
) -> None: ...
