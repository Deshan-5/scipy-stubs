...
_NumberT_co = TypeVar(
    "_NumberT_co", bound=npc.number, default=np.float64, covariant=True
)
_XPT_co = TypeVar("_XPT_co", default=ModuleType, covariant=True)

_IntegrandFunc: TypeAlias = Callable[
    Concatenate[onp.Array2D[_NumberT], ...], onp.ArrayND[npc.number]
]
...
class FixedRule(Rule[_XPT_co], Generic[_XPT_co, _NumberT_co]):  # undocumented
    ...
    def nodes_and_weights(
        self,
    ) -> tuple[onp.ArrayND[_NumberT_co], onp.ArrayND[_NumberT_co]]: ...  # abstract

class NestedFixedRule(
    FixedRule[_XPT_co, _NumberT_co], Generic[_XPT_co, _NumberT_co]
):  # undocumented
    ...
    @property
    def lower_nodes_and_weights(
        self,
    ) -> tuple[onp.ArrayND[_NumberT_co], onp.ArrayND[_NumberT_co]]: ...  # semi-abstract

class ProductNestedFixed(
    NestedFixedRule[_XPT_co, _NumberT_co], Generic[_XPT_co, _NumberT_co]
):
    base_rules: Sequence[NestedFixedRule[_XPT_co, _NumberT_co]]
    @override
    def __init__(
        self, /, base_rules: Sequence[NestedFixedRule[_XPT_co, _NumberT_co]]
    ) -> None: ...

def _cartesian_product(
    arrays: Iterable[onp.ArrayND[_NumberT]],
) -> onp.Array2D[_NumberT]: ...  # undocumented
def _split_subregion(
    a: onp.ArrayND[_NumberT],
    b: onp.ArrayND[_NumberT],
    xp: ModuleType,
    split_at: onp.ArrayND[_NumberT] | None = None,
