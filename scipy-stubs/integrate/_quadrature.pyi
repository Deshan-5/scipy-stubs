...
def trapezoid(
    y: onp.Array[_JustAnyShape, npc.number | np.bool_],
...
@overload  # 1d T:inexact
def romb(
    y: onp.Array1D[_Inexact80T], dx: float = 1.0, axis: int = -1, show: bool = False
) -> _Inexact80T: ...
@overload  # 2d +int
def romb(
    y: onp.ToFloat64Strict2D, dx: float = 1.0, axis: int = -1, show: bool = False
) -> onp.Array1D[np.float64]: ...
@overload  # 2d ~complex
def romb(
    y: onp.ToJustComplex128Strict2D | onp.ToJustComplex64Strict2D,
    dx: float = 1.0,
    axis: int = -1,
    show: bool = False,
) -> onp.Array1D[np.complex128]: ...
@overload  # 2d T:inexact
def romb(
    y: onp.Array2D[_Inexact80T], dx: float = 1.0, axis: int = -1, show: bool = False
) -> onp.Array1D[_Inexact80T]: ...
@overload  # Nd +int
def romb(
    y: onp.ToFloat64_ND, dx: float = 1.0, axis: int = -1, show: bool = False
) -> onp.ArrayND[np.float64] | Any: ...
@overload  # Nd ~complex
def romb(
    y: onp.ToJustComplex128_ND | onp.ToJustComplex64_ND,
    dx: float = 1.0,
    axis: int = -1,
    show: bool = False,
) -> onp.ArrayND[np.complex128] | Any: ...
@overload  # Nd T:inexact
def romb(
    y: onp.ArrayND[_Inexact80T], dx: float = 1.0, axis: int = -1, show: bool = False
) -> onp.ArrayND[_Inexact80T] | Any: ...
@overload  # +float (fallback)
def romb(
    y: onp.ToFloatND, dx: float = 1.0, axis: int = -1, show: bool = False
) -> onp.ArrayND[np.float64 | Any] | Any: ...
@overload  # +complex (fallback)
def romb(
    y: onp.ToComplexND, dx: float = 1.0, axis: int = -1, show: bool = False
) -> onp.ArrayND[np.complex128 | Any] | Any: ...
...
def cumulative_trapezoid(
    y: (
        onp.ToJustFloatND | onp.ToJustComplexND
    ),  # `ToComplexND` would overlap with the first overload
    x: onp.ToFloatND | None = None,
    dx: float = 1.0,
    axis: int = -1,
    initial: Literal[0] | None = None,
...
@overload  # (1d f64) -> +f64
def fixed_quad(
    func: _FixedQuadFunc[onp.ToFloat64_ND],
    a: float,
    b: float,
    args: tuple[object, ...] = (),
    n: int = 5,
...
def fixed_quad(
    func: _FixedQuadFunc[onp.ArrayND[_Inexact80T]],
    a: float,
    b: float,
    args: tuple[object, ...] = (),
    n: int = 5,
...
@overload  # (1d f64) -> ~m64
def fixed_quad(
    func: _FixedQuadFunc[onp.ArrayND[np.timedelta64]],
    a: float,
    b: float,
    args: tuple[object, ...] = (),
    n: int = 5,
...
def fixed_quad(
    func: _FixedQuadFunc[onp.ArrayND[np.object_]],
    a: float,
    b: float,
    args: tuple[object, ...] = (),
    n: int = 5,
