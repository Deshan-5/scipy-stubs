...
def convert_dtypes(
    dtype_template: Mapping[str, str], order_code: ByteOrder
) -> dict[str, np.dtype[Any]]: ...
def read_dtype(mat_stream: IO[bytes], a_dtype: npt.DTypeLike) -> onp.ArrayND: ...
def matfile_version(
    file_name: FileName, *, appendmat: bool = True
) -> tuple[Literal[0, 1, 2], int]: ...
def matdims(
    arr: onp.ArrayND, oned_as: Literal["column", "row"] = "column"
) -> tuple[Any, ...]: ...
def arr_dtype_number(
    arr: onp.HasDType[np.dtype[np.character]], num: int | str
