...
_MetricFunc: TypeAlias = Callable[
    [onp.Array1D[np.float64], onp.Array1D[np.float64]], onp.ToFloat | None
]
_Metric: TypeAlias = _MetricName | _MetricFunc  # noqa: PYI047

_Force: TypeAlias = Literal[
    "NO",
    "No",
    "no",
    "TOMATRIX",
...
@overload
def pdist(
    X: onp.ToFloat2D, metric: _MetricFunc, *, out: None = None, **kwargs: object
) -> _FloatingND: ...
@overload
def pdist(
    X: onp.ToComplex2D, metric: _MetricFunc, *, out: None = None, **kwargs: object
) -> _InexactND: ...
@overload
def pdist(
    X: onp.ToComplex2D, metric: _MetricFunc, *, out: _ArrayT, **kwargs: object
) -> _ArrayT: ...

#
@overload  # 1-d int
def squareform(
    X: onp.ToJustIntStrict1D, force: _Force = "no", checks: bool = True
) -> onp.Array2D[np.int_]: ...
@overload  # 1-d float
def squareform(
    X: onp.ToJustFloatStrict1D, force: _Force = "no", checks: bool = True
) -> onp.Array2D[np.float64]: ...
@overload  # 1-d complex
def squareform(
    X: onp.ToJustComplexStrict1D, force: _Force = "no", checks: bool = True
...
@overload  # 2-d int
def squareform(
    X: onp.ToJustIntStrict2D, force: _Force = "no", checks: bool = True
) -> onp.Array1D[np.int_]: ...
@overload  # 2-d float
def squareform(
    X: onp.ToJustFloatStrict2D, force: _Force = "no", checks: bool = True
) -> onp.Array1D[np.float64]: ...
@overload  # 2-d complex
def squareform(
    X: onp.ToJustComplexStrict2D, force: _Force = "no", checks: bool = True
...
#
def cosine(
    u: onp.ToFloat1D, v: onp.ToFloat1D, w: onp.ToFloat1D | None = None
) -> np.float64: ...

#
@overload
def mahalanobis(
    u: onp.ToFloat1D, v: onp.ToFloat1D, VI: onp.ToFloat2D
) -> np.float64: ...
@overload
def mahalanobis(
    u: onp.ToComplex1D, v: onp.ToComplex1D, VI: onp.ToComplex1D
...
@overload
def sokalsneath(
    u: onp.ToComplex1D, v: onp.ToComplex1D, w: onp.ToFloat1D | None = None
) -> np.float64 | np.complex128: ...
...
def jaccard(
    u: onp.ToComplex1D, v: onp.ToComplex1D, w: onp.ToBool1D | None = None
) -> np.float64: ...
def minkowski(
    u: onp.ToComplex1D, v: onp.ToComplex1D, p: float = 2, w: onp.ToFloat1D | None = None
) -> float: ...
def rogerstanimoto(
    u: onp.ToComplex1D, v: onp.ToComplex1D, w: onp.ToFloat1D | None = None
) -> float: ...
