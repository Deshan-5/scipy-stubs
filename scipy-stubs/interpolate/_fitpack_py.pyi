...
@overload  # tck: 2-d
def splev(
    x: onp.ToFloatND, tck: _ToTCK2D, der: int = 0, ext: _Ext = 0
) -> list[_FloatND]: ...
@overload  # tck: ?-d
def splev(
    x: onp.ToFloatND, tck: _ToTCK, der: int = 0, ext: _Ext = 0
) -> _FloatND | list[_FloatND]: ...

#
@overload  # tck: BSpline, full_output: falsy
def splint(
    a: onp.ToFloat, b: onp.ToFloat, tck: BSpline, full_output: onp.ToFalse = 0
) -> _Float | _Float1D: ...
@overload  # tck: BSpline, full_output: truthy
def splint(
    a: onp.ToFloat, b: onp.ToFloat, tck: BSpline, full_output: onp.ToTrue
) -> tuple[_Float | _Float1D, _Float1D]: ...
@overload  # tck: 1-d, full_output: falsy
def splint(
    a: onp.ToFloat, b: onp.ToFloat, tck: _ToTCK1D, full_output: onp.ToFalse = 0
) -> _Float: ...
@overload  # tck: 2-d, full_output: falsy
def splint(
    a: onp.ToFloat, b: onp.ToFloat, tck: _ToTCK2D, full_output: onp.ToFalse = 0
) -> list[_Float]: ...
...
def splint(
    a: onp.ToFloat, b: onp.ToFloat, tck: _ToTCK1D, full_output: onp.ToTrue
) -> tuple[_Float, _Float1D]: ...
@overload  # tck: 2-d, full_output: truthy
def splint(
    a: onp.ToFloat, b: onp.ToFloat, tck: _ToTCK2D, full_output: onp.ToTrue
) -> tuple[list[_Float], _Float1D]: ...
@overload  # tck: ?-d, full_output: truthy
def splint(
    a: onp.ToFloat, b: onp.ToFloat, tck: _ToTCK, full_output: onp.ToTrue
) -> tuple[_Float | list[_Float], _Float1D]: ...
...
def spalde(
    x: onp.ToFloatStrict1D, tck: _ToTCK
) -> list[_Float1D] | list[list[_Float1D]]: ...
...
def spalde(
    x: onp.ToFloatStrict2D, tck: _ToTCK
) -> list[list[_Float1D]] | list[list[list[_Float1D]]]: ...
@overload  # catch-all
def spalde(
    x: onp.ToFloatND, tck: _ToTCK
) -> _Float1D | list[_Float1D] | list[list[_Float1D]] | list[list[list[Any]]]: ...

#
@overload  # tck: BSpline
def insert(
    x: onp.ToFloat, tck: BSpline, m: int = 1, per: onp.ToBool = 0
) -> BSpline[np.float64]: ...
@overload  # tck: 1-d
def insert(
    x: onp.ToFloat, tck: _ToTCK1D, m: int = 1, per: onp.ToBool = 0
) -> _TCK1D: ...
@overload  # tck: 2-d
def insert(
    x: onp.ToFloat, tck: _ToTCK2D, m: int = 1, per: onp.ToBool = 0
) -> _TCK2D: ...
@overload  # tck: ?-d
def insert(
